...
console.log('prompt                          span  covered  dropped  supports  added  pieces  decodes');
for (const prompt of EXAMPLES) {
  if (only && !prompt.includes(only)) continue;
  const r = carveSculpture(qr.bitmap, qr.quietZone, qr.moduleCount, sculpt(prompt)!.sdf,
    { span: 1, zSub: 2, xySub: 3, tileLayers: 2, selfSupport: true });
  const ok = verifyTopView(r.code, payload).matches;
  console.log(
    prompt.slice(0, 30).padEnd(31), String(r.spanModules).padStart(4), `${(r.coverageFraction * 100).toFixed(0)}%`.padStart(8),
